...
let autoTimer = null;
let _pendingOverride = null; // override en attente si le résultat arrive pendant l'animation

// ─── Container ────────────────────────────────────────────────────────────────
// Doit exister dans le DOM avec des dimensions réelles AVANT d'appeler init().
// On le cache via opacity, PAS via display:none (qui provoquerait un canvas 0×0).
...
async function initDiceBox() {
  ...
  // API v1.1.0+ : un seul objet config avec la propriété "container"
...
/** Overlay simple : grand chiffre seul (dé libre sans modificateur). */
...
/**
 * Overlay enrichi : chips de dés + chip modificateur + total.
 * Utilisé quand il y a un bonus non nul ou plusieurs dés (avantage/désavantage).
 */
function buildRollResultHTML(sides, baseRoll, rolls, isCritical, isFumble, bonus, rollLabel) {
  ...
  // Chips pour chaque dé (1 ou 2 pour avantage/désavantage)
...
/**
 * Lance l'animation 3D et renvoie le résultat RÉEL du dé (celui qui s'affiche sur la face).
 *
 * @param  {number} sides
 * @param  {string} rollMode   "normal" | "advantage" | "disadvantage"
 * @param  {number} bonus
 * @param  {string} rollLabel
 * @param  {string} [themeColor]  Couleur hex du dé (ex: "#7c3aed")
 * @returns {Promise<{ baseRoll: number, rolls: number[], mode: string }>}
 */
/**
 * Génère des valeurs de dés pour un diceMap AVANT animation.
 * Utilisé pour synchroniser les mêmes faces sur tous les navigateurs.
...
function buildMultiResultHTML(diceRolls, total, flatModifier = 0) {
  // Cas sans dés — valeur fixe uniquement
 ...
 * @param  {boolean}              [isRemote]    Si true : non-bloquant (pas de clic requis, pointer-events:none)
...
export async function triggerMultiDiceAnimation(diceMap, flatModifier = 0, overrideRolls = null, themeColor = "#7c3aed", isRemote = false) {
  ...
  // Configs dés — themeColor par jet, valeurs laissées au moteur physique
  ...
  // Priorité : override en attente (résultat reçu pendant l'animation) > overrideRolls > physique
  ...
  if (isRemote) {
    // Mode distant : pas de click requis pour fermer, sécurité 30s si le popup ne ferme pas
 ...
 * - Overlay déjà visible  → mise à jour immédiate
 * - Animation en cours    → stocke pour l'appliquer dès la stabilisation
 */
...
/** Ferme l'animation 3D en cours (locale ou distante). */
